...
def build_model_test(dim1,dim2,roberta_layer):
    ...
    mha = MultiHeadAttention(num_heads=6, key_dim=96)
    cross_attended_1 = mha(x, y, y) 
    cross_attended_2 = mha(y, x, x) 
    cross_attended_1 = layers.Dropout(0.1)(cross_attended_1)  
    cross_attended_2 = layers.Dropout(0.1)(cross_attended_2)  
    ...
    out = layers.Dense(192,activation='relu')(out)
    out = Dropout(0.4)(out)
...
def train_model(train_motif, train_motif_mask, train_seq, train_seq_mask, train_label, model_name, model_path, output_filename):
    ...
    model.compile(optimizer=optimizers.RMSprop(lr=1.4353e-05, epsilon=1e-08),
...
def test_model(test_motif,test_motif_mask,test_seq,test_mask, model_name, model_path,result_path):
    pred_prob = None
    results_df = pd.DataFrame()  # 用于保存预测概率
    ...
    pro_file=result_path+ model_name + '.npy'
...
if __name__ == "__main__":
    ...
    train_file = f'train_data_ST.xlsx'
